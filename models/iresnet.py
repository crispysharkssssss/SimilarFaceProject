...
from .cbam import CBAM  # 导入刚才写的模块
...
class IResNet(nn.Module):
    ...
    def __init__(self, block, layers, dropout=0, num_features=512, zero_init_residual=False,
        ...
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0], use_cbam=use_cbam)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1], use_cbam=use_cbam)
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2,
                                       dilate=replace_stride_with_dilation[2], use_cbam=use_cbam)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False, use_cbam=False):
        ...
        layers.append(block(self.inplanes, planes, stride, downsample, self.groups,
                            self.base_width, previous_dilation, use_cbam=use_cbam))
